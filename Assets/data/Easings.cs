...
static public class Easings
{
    ...
    {
        return (float)Math.Sin(p * (float)HALFPI);
    }
    ...
    static public float CircularEaseInOut(float p)
    {
        ...
        {
            return 0.5f * ((float)Math.Sqrt(-((2f * p) - 3f) * ((2f * p) - 1f)) + 1f);
        }
    }
    ...
    {
        return (p == 0.0f) ? p :(float)Math.Pow(2f, 10f * (p - 1f));
    }
    ...
    {
        return (p == 1.0f) ? p : 1 -(float)Math.Pow(2f, -10f * p);
    }
    ...
    {
        return (float)Math.Sin(13 * (float)HALFPI * p) *(float)Math.Pow(2, 10 * (p - 1));
    }
    ...
    {
        return (float)Math.Sin(-13 * (float)HALFPI * (p + 1)) *(float)Math.Pow(2, -10 * p) + 1;
    }
    ...
    static public float ElasticEaseInOut(float p)
    {
        if(p < 0.5f)
        {
            return 0.5f * (float)Math.Sin(13 * (float)HALFPI * (2 * p)) *(float)Math.Pow(2, 10 * ((2 * p) - 1));
        }
        else
        {
            return 0.5f * ((float)Math.Sin(-13 * (float)HALFPI * ((2 * p - 1) + 1)) *(float)Math.Pow(2, -10 * (2 * p - 1)) + 2);
        }
    }
